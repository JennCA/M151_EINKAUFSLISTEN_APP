...
class Comments
{
    ...
    public function getCommentsByItemId(int $itemId)
    {
        ...
        $statement = $connection->prepare('SELECT id, comment, itemId, userId FROM jfc_comments WHERE itemId = ?');
    ...
    {
        $connection = $this->connection();
        $statement = $connection->prepare('INSERT INTO jfc_comments (comment, itemId, userId) VALUES (?, ?, ?)');
